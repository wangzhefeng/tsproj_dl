...
# * Date        : 2026-03-28
# * Version     : 1.0.032820
# * Description : Time-Series-Library Koopa model adapted for local config style
# ***************************************************
...
class Model(nn.Module):
    ...
    def __init__(self, configs, dynamic_dim=128, hidden_dim=64, hidden_layers=2, num_blocks=3, multistep=False):
        ...
        self.num_blocks = getattr(configs, "num_blocks", num_blocks)
        self.dynamic_dim = getattr(configs, "dynamic_dim", dynamic_dim)
        self.hidden_dim = getattr(configs, "koopman_hidden_dim", hidden_dim)
        self.hidden_layers = getattr(configs, "koopman_hidden_layers", hidden_layers)
        self.multistep = multistep
        self.alpha = getattr(configs, "alpha", 0.2)
