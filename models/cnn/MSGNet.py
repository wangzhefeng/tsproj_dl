...
# * Date        : 2026-03-28
# * Version     : 1.0.032820
# * Description : Time-Series-Library MSGNet model adapted for local config style
# ***************************************************
...
class ScaleGraphBlock(nn.Module):
    def __init__(self, configs):
        ...
        conv_channel = getattr(configs, "conv_channel", configs.d_model)
        skip_channel = getattr(configs, "skip_channel", configs.d_model)
        gcn_depth = getattr(configs, "gcn_depth", 2)
        propalpha = getattr(configs, "propalpha", 0.05)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # self.subgraph_size = getattr(configs, "subgraph_size", 5)
        # self.node_dim = getattr(configs, "node_dim", 16)
        ...
        self.seq2pred = Predict(getattr(configs, "individual", False), configs.c_out,
