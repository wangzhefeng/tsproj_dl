...
export LOG_NAME="${LOG_NAME:-smoke-etth1-transformer}"

"$PYTHON_BIN" -u "$ROOT_DIR/run_dl.py" \
    --task_name long_term_forecast \
    --des "Smoke Transformer ETTh1" \
    --is_training 1 \
    --is_testing 0 \
    --is_forecasting 0 \
    --model_id smoke_etth1_transformer \
    --model Transformer \
    ...
    --target OT \
    --time date \
    --results_root "$ROOT_DIR/results/smoke_core005/" \
    --freq 1h \
    --embed timeF \
    --seq_len 24 \
    --label_len 12 \
